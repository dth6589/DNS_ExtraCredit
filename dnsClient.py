...
def dns_query(type, name, server):
    ...
    qname_parts = name.split('.') # How can we easily split the string? Split on dots
    qname_encoded_parts = [struct.pack('B', len(part)) + part.encode('ascii') for part in qname_parts] # Make sure it's encoded as a sequence of the right character encoding type (lowercase) | ASCII?
    qname_encoded = b''.join(qname_encoded_parts) + b'\x00' #enter the closing byte value to signify the end of the domain string (two digits) | 00

    # Encode the QTYPE and QCLASS

    if type == 'A':
        qtype = 1     # Lookup the Resource Record value | RFC1035
    elif type == 'AAAA':
        qtype = 28    # Lookup the Resource Record value | RFC1035
    else:
        raise ValueError('Invalid type')
    

    qclass = 1     # Lookup the Resource Record class being requested | In in RFC1035
